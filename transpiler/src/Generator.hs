...
-- Generator for names -- should ensurehtat dots are replaced with underscores
...
generateStmt (SSwitch e cs) i = 
  let sel = generateExp e
   ...
   in i ++ "switch " ++ sel ++ " {\n" ++ opt ++ "\n}" 
--generateStmt (SGo "par" ss) i =
--  let wg = randomString OnlyAlphaNum 4
--      ind = randomString OnlyAlphaNum 4
--      pre = i ++ "var " ++ wg ++ " sync.WaitGroup\n"
--      sts = map (\x -> generateStmt x (i ++ "    ")) ss
--      for = unlines [concat [i, "for ", ind, " := 0; ", ind, " < len(", sts, "); ", ind, "++{"],
--                     i ++ "  " ++ wg ++ ".Add(1)",
--                     i ++ "  go func() {",
--                     i ++ "    defer " ++ wg ++ ".Done()",
--                     sts[ind],
...
generateProg [FFun name args [] stmt] = let n = generateName name
                                            main = unlines ["func main() {",
                                                            ...
                                                            "  }",
                                                            "}"]
                                            fun = generateFun (FFun n args [] stmt)
